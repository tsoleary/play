...
nik <- read.csv(file = "percent_covers_Niku.csv", header = TRUE)
oro <- read.csv(file = "percent_covers_Orona.csv", header = TRUE)
raw <- read.csv(file = "percent_covers_Rawaki.csv", header = TRUE) 

# combine all your sites into a single df
df <- rbind(nik, oro, raw)  

# remove non-sense columns
df <- select(df, -c("Annotation.status", "Annotation.area"))
...
corals <- c("Acropora", "Acrop", "Astreo", "Cypha", "Ech", "ECHIN", "Fav", 
            "Favia", "Fung", "GAL", "Gon", "Hal", "HYDNO", "Lept", "Lepta", 
            "Lepto", "Lobo", "Mer", "MER", "Monta", "Monti", "Myc", "Oxyp", 
            "Pachy", "Pavon", "Plat", "Pocill", "Porit", "Psam", "Turbi")
iverts <- c("Sponge", "ZOAN")
algae <- c("AVRA", "BSP", "Caul", "Dicsp", "X.DICT", "GALA", "Lobph", "Turf_algae")
calc_algae <- c("CCA.1", "Hali", "Peysson")
...
make_func_groups <- function (dat){
  ...
  for (i in 1:length(dat$Genus)) {
    g <- dat$Genus[i]
    if ( is.element(g, corals) ) {
      funct_groups <- c(funct_groups, "Coral")
    } else if ( is.element(g, iverts) ) {
      funct_groups <- c(funct_groups, "Invert")
    } else if ( is.element(g, algae) ) {
      funct_groups <- c(funct_groups, "Algae")
    } else if ( is.element(g, calc_algae) ) {
      funct_groups <- c(funct_groups, "Calc.Algae")
    } else if ( g == "Soft" ) {
      funct_groups <- c(funct_groups, "Soft")
    } else if ( g == "Sand" ) {
      funct_groups <- c(funct_groups, "Sand")
    } else {
...
# TSO: you have to sum the functional group then average the sums! :)
fun_gp_means <- df_tidy %>% 
  ...
  summarize(N = sum(!is.na(sum)),
            Avg = mean(sum, na.rm = TRUE),
...
plots_to_pdf(coral_plots, "coral_plots_corrected.pdf") # file written in working directory

funct_plots <- plot_all(fun_gp_means, FUN = plot_funct_group)
plots_to_pdf(funct_plots, "funct_plots_corrected.pdf") # file written in working directory
